  ...
  const fetchRestaurants = async (keyword: string = '') => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/places?lat=${LAT}&lng=${LNG}&query=${encodeURIComponent(query)}`
      );
              ...
              {restaurants.map((restaurant, idx) => (
                <button
                  ...
                  className={`w-full text-left p-4 hover:bg-orange-50 transition-all
                    ${selectedRestaurant?.place_id === restaurant.place_id ? 'bg-orange-50 border-l-4 border-orange-500' : ''}`}
                >
    ...
    {restaurant.distance && (
      <span className="text-xs text-gray-400">
        ...
        <div className="flex-1 relative">
          ...
          {/* 선택된 식당 정보 오버레이 */}
          {selectedRestaurant && (
            <div className="absolute bottom-4 left-4 right-4 bg-white rounded-2xl shadow-xl p-4 border border-orange-100">
              <div className="flex gap-3">
                {selectedRestaurant.photos?.[0] && (
                  <div className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0">
                    <img
                      src={getPhotoUrl(selectedRestaurant.photos[0].photo_reference)}
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-black text-gray-800 truncate">{selectedRestaurant.name}</h3>
                  <p className="text-xs text-gray-400 truncate mt-0.5">📍 {selectedRestaurant.vicinity}</p>
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-xs text-yellow-500 font-bold">⭐ {selectedRestaurant.rating}</span>
...
{selectedRestaurant.distance && (
  <span className="text-xs text-gray-400">
    🚶 회사에서 {selectedRestaurant.distance}m · 약 {Math.ceil(selectedRestaurant.distance / 67)}분
  </span>
)}
                  </div>
                </div>
                <button
                  ...
                  className="flex-shrink-0 text-xs bg-orange-500 text-white px-3 py-1.5 rounded-xl font-bold self-start"
                >
                  리뷰 보기
                </button>
              </div>
        ...
        {showReviewForm && selectedRestaurant && (
          <div className="w-72 bg-white border-l border-gray-100 overflow-y-auto p-4">
            <div className="flex items-center justify-between mb-4">
              <p className="font-bold text-gray-800">팀원 리뷰</p>
              <button onClick={() => setShowReviewForm(false)} className="text-gray-400 text-xs">닫기</button>
            </div>

            {/* 리뷰 작성 */}
            {!myReviewMap[selectedRestaurant.place_id] && (
              <div className="bg-orange-50 rounded-xl p-3 mb-4 space-y-2">
            ...
            <div className="space-y-3">
              {reviews.length > 0 ? reviews.map(review => (
                  ...
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {review.user_id === user.id && (
                        <button onClick={() => handleDeleteReview(review.id)} className="text-xs text-gray-300 hover:text-red-400">삭제</button>
                      )}
                    </div>
                  </div>
                ...
                <p className="text-xs text-gray-300 text-center py-4">아직 리뷰가 없어요!</p>
              )}
            </div>
          </div>
        )}
